...
contract TokenFactoryTest is Test {
    ...
    uint256 public constant USDT_MINT = 200e6; // 200 USDT with 6 decimals
    ...
    function setUp() external {
        ...
        ERC20Mock(address(usdt)).mint(user, USDT_MINT);
        vm.prank(user);
        usdt.approve(address(tokenFactory), type(uint256).max);
    }
